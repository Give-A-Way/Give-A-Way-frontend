...
const Details = (props) => {
  ...
  useEffect(() => {
    setMinDayToDonate(
      ...
      }-${cuurentDate.getDate()}T${cuurentDate.getUTCHours()}:${cuurentDate.getMinutes()}`
    );
  ...
  const changeStatus = (e) => {
    ...
    let myTime =
      "" +
      (dateWithInitialValue.$H < 10
        ? `0${dateWithInitialValue.$H}`
        : dateWithInitialValue.$H) +
      ":" +
      (dateWithInitialValue.$m < 10
        ? `0${dateWithInitialValue.$m}`
        : dateWithInitialValue.$m);
    SetsubmissionData({
      type: e.target[1].value,
      description: e.target[3].value,
      time: myTime,
      day: `${dateWithInitialValue.$M + 1}/${dateWithInitialValue.$D}/${
        dateWithInitialValue.$y
      }`,
    });
    rain();
      ...
      <SidePanel>
        ...
        <FormHolder>
          ...
          <label for="radio3">Radio 3</label>
            ...
            <fieldset>
              <label style={{ color: "white" }} htmlFor="select-choice">
                Type of Donation
              </label>
              <select
                className="cd-select"
                ...
                id="select-choice"
              >
                <option key="donation-type-choice-1" value="Choice 1">
                  - - select one - -
                </option>
                <option key="donation-type-choice-2" value="Food">
                  Food
                </option>
                <option key="donation-type-choice-3" value="Drinks">
                  Drinks
                </option>
                <option key="donation-type-choice-4" value="Clothes">
                  Clothes
                </option>
                <option key="donation-type-choice-6" value="Money">
                  Money
                </option>
                <option key="donation-type-choice-5" value="Other">
                  Other
                </option>
              </select>
            </fieldset>
            ...
            <fieldset>
              <label style={{color: "white"}}  htmlFor="textarea">Description of Donation</label>
              <textarea
                className="cd-textarea"
                name="textarea"
                id="textarea"
                placeholder="Thank you for your generousity!"
              ></textarea>
            </fieldset>
